"""
From https://github.com/circlePi/Bert_Chinese_Ner_pytorch
"""
...
class CRF(nn.Module):
    def __init__(self,num_tag):
        ...
        self.transitions.data[self.end_tag, :] = -10000  # 表示从EOS->其他标签为不可能事件, 如果发生，则产生个极大的损失
    ...
    def negative_log_loss(self, inputs, tags, label_mask):
        """
        ...
        num_chars = torch.sum(label_mask.detach()).float()  # 所有字的个数
        for ix, (features, mask, tag) in enumerate(zip(inputs, label_mask, tags)):
            valid_length = torch.nonzero(mask).shape[0]
            features = features[:valid_length]
            tag = tag[:valid_length]
            real_score = self.real_path_score(features, tag)  # 真实路径分数
            total_score = self.all_possible_path_score(features)  # 所有可能的路径分数
    ...
    def get_batch_best_path(self, inputs, output_mask):
        ...
        batch_best_path = torch.cat(batch_best_path, dim=0)
